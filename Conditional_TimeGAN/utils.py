...
# def batch_generator(data, time, batch_size):        #여기 수정 필요할듯??????
...
# def batch_generator(data, time, categories, batch_size):
#   """Mini-batch generator with category labels.
...
#     - X_mb: time-series data in each batch
#     - T_mb: time information in each batch
#     - C_mb: category labels in each batch
#   """
#   no = len(data)
#   idx = np.random.permutation(no)
#   train_idx = idx[:batch_size]
...
#   C_mb = list(categories_one_hot[i] for i in train_idx)
...
def batch_generator(data, time, categories, batch_size):
    """Mini-batch generator without shuffling.
    ...
    no = len(data)
    # 순서대로 데이터를 선택
    X_mb = list(data[i] for i in range(batch_size))
    T_mb = list(time[i] for i in range(batch_size))
    # One-hot encoding of categories
    num_categories = len(np.unique(categories))
    categories_one_hot = np.eye(num_categories)[categories]
    C_mb = list(categories_one_hot[i] for i in range(batch_size))
