...
plot_save_path = '/home/langleman/TimeGAN4paper/result/1204_4paper/figures'
csv_save_path = '/home/langleman/TimeGAN4paper/result/1204_4paper/LSTM_2022_weekly_1204_4paper.csv'
...
def category_data_multi3(data, cate, sequence_length, lag):
    d = data[data['item_category']==cate]
    d = d[['판매수량']]
    ...
    y_seq = torch.FloatTensor(y_seq).view([-1, lag]) # float형 tensor로 변형

    df = pd.read_csv(f'/home/langleman/TimeGAN4paper/generated_data/data4inference/padded_weekly_{cate}.csv')
    num_of_generated_data, length = df.shape

    
    total_x1_seq = None
    total_y1_seq = None
    for i in range(num_of_generated_data):
        x1 = df.loc[i].values.reshape(length,1)
        y1 = df.loc[i].values
        x1_seq = []
        y1_seq = []
        for i in range(len(x1) - sequence_length):
            if len(y1[i+sequence_length:(i+sequence_length+lag)])==lag:
                x1_seq.append(x1[i: (i+sequence_length+lag-1)])
                y1_seq.append(y1[i+sequence_length:(i+sequence_length+lag)])
            else:
                break
        x1_seq = torch.FloatTensor(x1_seq)
        y1_seq = torch.FloatTensor(y1_seq).view([-1, lag]) # float형 tensor로 변형

        if total_x1_seq is None:
            total_x1_seq = x1_seq
            total_y1_seq = y1_seq

        else:
            total_x1_seq = torch.cat((total_x1_seq, x1_seq), dim = 0)
            total_y1_seq = torch.cat((total_y1_seq, y1_seq), dim=0)

            total_x1_seq = torch.FloatTensor(total_x1_seq)
            total_y1_seq = torch.FloatTensor(total_y1_seq).view([-1, lag]) # float형 tensor로 변형
        
    print(total_y1_seq.shape)
   
    return torch.cat((total_x1_seq, x_seq), dim = 0), torch.cat((total_y1_seq, y_seq), dim=0)
...
def train_and_plot(data, i, test_size = 12, sequence_length = 12, lag = 1, input_size = 6, hidden_size = 64, num_layers = 1,
    ...
    model_save_path = '/home/langleman/TimeGAN4paper/result/1204_4paper/model/{}_lag{}_total1/'.format(i, lag)
    os.makedirs(model_save_path, exist_ok=True)
    x_seq, y_seq = category_data_multi3(data, i, sequence_length, lag)
...
def main():
    ...
    #epoch_dict = {'BL' : 1, 'PD' : 1, 'CT' : 8, 'JP' : 1, 'KC' : 2, 'TC' :1, 'SK' : 3, 'VT' : 4, 'KT' : 8, 'OP' : 1, 'JK' : 200, 'SL' : 7, 'TS' : 1}
    ...
    #epoch_dict = {'BL' : 1, 'PD' : 15000, 'CT' : 80000, 'JP' : 1, 'KC' : 20000, 'TC' :10000, 'SK' : 30000, 'VT' : 1, 'KT' : 80000, 'OP' : 10000, 'JK' : 1, 'SL' : 70000, 'TS' : 100000}
