...
for cat in category:
    ...
    # %%
    df = pd.read_csv(f'/home/langleman/TimeGAN4paper/generated_data/data/generated_data_new_{cat}_1231.csv')
    df.shape
    ...
    folder_path = '/home/langleman/TimeGAN4paper/generated_data/data4inference'

    df.to_csv(os.path.join(folder_path, f'padded_weekly_{cat}_1231.csv'), index=False)


    vars()[f'pad_{cat}'] = pd.read_csv(os.path.join(folder_path, f'padded_weekly_{cat}_1231.csv'))

    fig,ax = plt.subplots(1)
    for i in range(12):
        vars()[f'pad_{cat}'].loc[i].plot(ax=ax,figsize = (16,9), color = 'b', alpha = 0.2, label='Generated Data' if i == 0 else "")
    ori_data.plot(ax=ax, color = 'g', label = 'Original Data')

        # 레전드 추가를 위한 Line2D 객체 생성
    blue_line = mlines.Line2D([], [], color='b', label='Generated Data')  # 불투명한 파란색
    green_line = mlines.Line2D([], [], color='g', label='Original Data')  # 초록색
    ...
    save_path = f'/home/langleman/TimeGAN4paper/generated_data/plot4all/plot4paper_241231/{cat}_plot_1231.png'
